...
| Tests | 885 across 28 suites, all green |
...
3. **Phase 4 remainder** — rasterize/vectorize, and wiring reprojection through
   the pipeline. **Contours, clip-by-polygon and the warp engine are done**
   (`engines/raster/contour.ts`, `clip.ts`, `warp.ts`). Contours and clip are
   reachable from the settings panel and the palette; `warp.ts` is engine-only
   so far — `transformDataset` now REPORTS that a raster was not reprojected
   rather than relabelling it silently, which is the honest interim state, and
   wiring the warp behind that warning is the next step.
