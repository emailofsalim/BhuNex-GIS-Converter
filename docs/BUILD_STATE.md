...
| Tests | 861 across 27 suites, all green |
...
3. **Phase 4 remainder** — resampling, raster reprojection, rasterize/vectorize.
   **Contours and clip-by-polygon are done** (`engines/raster/contour.ts`,
   `engines/raster/clip.ts`), both wired through the pipeline and reachable from
   the settings panel and the command palette. The GeoTIFF codec reads and
   writes already; the rest is the analysis layer on top of it.
