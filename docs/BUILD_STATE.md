...
1. **Phase 11 remainder (§25, §26).** Measurement, snapping and the vertex
   editor are built AND wired: the Edit tab drags vertices on the canvas with a
   live readout, and every change lands in the history. Next: the attribute
   table (§25.5), the layer manager (§25.4) and the geometry operations of
   §26.2 — plus a measurement tool on the canvas, which the engine already
   supports.
