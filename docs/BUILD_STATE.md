...
1. **Phase 11 remainder (§26.2).** Measurement, snapping, the vertex editor,
   the attribute table (§25.5) and the layer manager (§25.4) are built AND
   wired, and workspace edits now reach the exported file (see `core/edits.ts`).
   Next: the geometry operations of §26.2 — buffer, offset, union, intersection,
   difference, convex hull, dissolve, clip, erase, split-by-line, line merge,
   explode — plus a measurement tool on the canvas, which the engine already
   supports.
