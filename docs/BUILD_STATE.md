...
| Tests | 888 across 28 suites, all green |
...
3. **Phase 4 remainder** — rasterize and vectorize. Everything else in the
   raster analysis layer is done and reachable: contours
   (`engines/raster/contour.ts`), clip-by-polygon (`clip.ts`) and resampling /
   reprojection (`warp.ts`, wired through the pipeline so a raster's PIXELS move
   when the dataset is reprojected). The one remaining shortcut is the inverse
   transform, which is found numerically by Newton's method rather than in
   closed form — exact for the affine case and well under a millimetre for the
   bundled projections, but a closed-form inverse would be faster and is the
