...
| Tests | 830 across 25 suites, all green |
...
3. **Phase 4 remainder** — resampling, raster reprojection, clip-by-polygon,
   rasterize/vectorize. **Contours are done** (`engines/raster/contour.ts`,
   wired through the pipeline and the settings panel). The GeoTIFF codec reads
   and writes already; the rest is the analysis layer on top of it.
