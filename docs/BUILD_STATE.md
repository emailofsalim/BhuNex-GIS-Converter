...
1. **Architecture conformance gaps.** `docs/CHROME_EDGE_ARCHITECTURE_SPEC.txt`
   is the supplied base architecture; `docs/ARCHITECTURE_CONFORMANCE.md` audits
   this codebase against all 40 of its rules — 34 met, 2 met under different
   naming, 4 genuine gaps. Fix them in this order, because the first unblocks
   the second:
   - **Worker pool (RULE 15).** `parallelJobs` starts N concurrent jobs but they
     all queue behind ONE worker. The setting promises parallelism it cannot
     deliver.
   - **Cancellation and progress (RULES 27, 28).** Neither exists.
   - **Benchmarks (RULE 31).** No `benchmarks/`. `StatusLine` (O(n) insert) and
     the buffer's pairwise `unionAll` want measuring before optimising.
   - **`workspace/main.ts` is 4,609 lines (RULE 32).** Split by panel.
